...
public class Door extends Entity{
	...
	public void update(float delta){
		if(opening){
			...
			sprite.setAlpha(getGame().alphaRatio());
		}
	}
	...
	private void teleportPlayers(float delta){
		...
		float roomSizeX = getGame().getLabyrinth().getRoomSize().x * 16;
		float roomSizeY = getGame().getLabyrinth().getRoomSize().y * 16;
		switch(direction){
		case UP:
			getGame().getLabyrinth().updateCurrentRoom(0, 1);
			if(players.size > 1){
			...
			break;
		case RIGHT:
			getGame().getLabyrinth().updateCurrentRoom(1, 0);
			if(players.size > 1){
